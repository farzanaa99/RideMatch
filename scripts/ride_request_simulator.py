    ...
    async with httpx.AsyncClient(timeout=args.timeout_seconds) as client:
        for i in range(args.count):
            if args.burst_every > 0 and i > 0 and i % args.burst_every == 0:
                ...
                tasks = [
                    post_ride(client, args.base_url, payload)
            ...
            try:
                status_code, elapsed_ms = await post_ride(
                    client,
                    args.base_url,
                    payload,
                )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Async ride request load simulator"
    )
    parser.add_argument(
        "--base-url",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Minimum inter-arrival time",
    )
    parser.add_argument(
        "--max-interval-seconds",
        type=float,
        default=0.25,
        help="Maximum inter-arrival time",
    )
    parser.add_argument(
        "--burst-every",
        type=int,
        default=0,
        help="Emit a burst every N requests (0 disables burst mode)",
    )
    parser.add_argument(
        "--burst-size",
        type=int,
        default=20,
        help="Requests in each burst",
    )
    parser.add_argument(
        "--burst-interval-seconds",
        type=float,
        default=2.0,
        help="Sleep between bursts",
    )
    parser.add_argument(
        "--lat-center",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=8.0,
        help="Coordinate jitter radius",
    )
    parser.add_argument(
        "--max-retries",
        type=int,
        default=3,
        help="max_retries field in generated rides",
    )
    parser.add_argument(
        "--timeout-seconds",
        type=float,
